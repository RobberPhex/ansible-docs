...
* This module does not support running commands in configuration mode. Please use :ref:`iosxr_config <iosxr_config>` to configure iosxr devices.
...
.. raw:: html
    ...
    <td></td>
        <td></td>
        <td><div>List of commands to send to the remote iosxr device over the configured provider. The resulting output from the command is returned. If the <em>wait_for</em> argument is provided, the module is not returned until the condition is satisfied or the number of retries has expired.</div>        </td></tr>
