...
This module does not support running commands in configuration mode. Please use :ref:`dellos10_config <dellos10_config>` to configure Dell OS10 devices.
...
.. raw:: html
    ...
    <td></td>
        <td><ul></ul></td>
        <td><div>List of commands to send to the remote dellos10 device over the configured provider. The resulting output from the command is returned. If the <em>wait_for</em> argument is provided, the module is not returned until the condition is satisfied or the number of retries has expired.</div></td></tr>
            <tr>
