...
Sends arbitrary commands to a Dell OS6 node and returns the results read from the device. The :ref:`dellos6_command <dellos6_command>` module includes an argument that will cause the module to wait for a specific condition before returning or timing out if the condition is not met.
