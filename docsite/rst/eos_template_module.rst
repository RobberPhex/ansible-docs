...
.. raw:: html
    ...
    <td></td>
        <td><ul><li>yes</li><li>no</li></ul></td>
        <td><div>This argument will cause the provided configuration to be replaced on the destination node.   The use of the replace argument will always cause the task to set changed to true and will implies <code>force=true</code>.  This argument is only valid with <code>transport=eapi</code>.</div></td></tr>
            <tr>
