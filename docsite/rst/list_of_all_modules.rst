...
.. toctree:: :maxdepth: 1
  ...
  dnf (E) - Manages packages with the I(dnf) package manager <dnf_module>
  ...
  ec2_ami_search - Retrieve AWS AMI information for a given operating system. <ec2_ami_search_module>
  ...
  fireball - Enable fireball mode on remote node <fireball_module>
  ...
  haproxy (E) - An Ansible module to handle states enable/disable server and set weight to backend host in haproxy using socket commands. <haproxy_module>
  ...
  known_hosts (E) - Add or remove a host from the C(known_hosts) file <known_hosts_module>
  ...
  snmp_facts (E) - Retrive facts for a device using SNMP. <snmp_facts_module>
  ...
  zabbix_group (E) - Add or remove a host group to Zabbix. <zabbix_group_module>
