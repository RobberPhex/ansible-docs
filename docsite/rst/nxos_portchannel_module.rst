...
.. note:: ``state=absent`` removes the portchannel config and interface if it already exists. If members to be removed are not explicitly passed, all existing members (if any), are removed.
.. note:: Members must be a list.
.. note:: LACP needs to be enabled first if active/passive modes are used.
