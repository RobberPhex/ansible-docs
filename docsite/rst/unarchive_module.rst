...
The :ref:`unarchive <unarchive>` module unpacks an archive. By default, it will copy the source file from the local system to the target before unpacking - set remote_src=yes to unpack an archive which already exists on the target..
...
.. raw:: html
    ...
    <td></td>
        ...
        <td><div>name of the user that should own the file/directory, as would be fed to <em>chown</em></div></td></tr>
            <tr>
    <td>remote_src<br/><div style="font-size: small;"> (added in 2.2)</div></td>
    <td>no</td>
    <td>no</td>
        <td><ul><li>yes</li><li>no</li></ul></td>
...
.. note:: can handle *.zip* files using ``unzip`` as well as *.tar*, *.tar.gz*, *.tar.bz2* and *.tar.xz* files using ``gtar``
