...
.. raw:: html
    ...
    <td></td>
        ...
        <td><div>The datacenter on the vCenter server that holds the datastore.</div></td></tr>
            <tr>
    <td>datastore<br/><div style="font-size: small;"></div></td>
    <td>yes</td>
    <td></td>
        <td><ul></ul></td>
...
.. note:: This module ought to be run from a system that can access vCenter directly and has the file to transfer. It can be the normal remote target or you can change it either by using ``transport: local`` or using ``delegate_to``.
